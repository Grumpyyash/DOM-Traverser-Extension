...
import { AppPresentation } from './densityPresentation'
import {
...
const App = (): JSX.Element => {
  ...
  // show path of the error element in modal
  const handleShowPath = useCallback((path: string): void => {
    setCurrentPath(path)
    setShowPath(true)
  }, [])

  // close modal which shows full path
  const handleClosePath = useCallback((): void => {
    setCurrentPath('')
    setShowPath(false)
  }, [])

  // push an alert to the dom for showing any messages
  ...
  // send message to content-script to highlight an element
